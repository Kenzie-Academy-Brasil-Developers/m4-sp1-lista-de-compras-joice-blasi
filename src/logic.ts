...
export const updateItemByName = (request: Request, response: Response) => {
    ...
    if (idExists) {
        ...
        if (list) {
            ...
            const nameExists = data.find(item => item.name === nameParam);
            if (nameExists) {
                const keysData = Object.keys(nameExists);
